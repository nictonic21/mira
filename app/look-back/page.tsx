...
export default function LookBack() {
  ...
  const loadEverything = async (userId: string) => {
    setIsLoading(true);
    await Promise.all([
      loadOnThisDay(userId),
      loadMonthlyMemories(userId),
      loadMoodJourney(userId),
      loadDreams(userId),
      loadMonthlyWrappedData(userId),
      loadYearlyWrappedData(userId),
      loadMiraRemembers(userId),
    ]);
    setIsLoading(false);
  };

  const loadOnThisDay = async (userId: string) => {
    ...
    for (let yearsAgo = 1; yearsAgo <= 5; yearsAgo++) {
      ...
      const { data: msgData } = await supabase.from("messages").select("*").eq("user_id", userId).eq("role", "user").gte("created_at", dateStr).lte("created_at", dateStr + "T23:59:59").limit(1);
      if (msgData && msgData.length > 0) {
        memories.push({ content: msgData[0].content, yearsAgo, type: "message" });
      }

      const { data: moodData } = await supabase.from("moods").select("*").eq("user_id", userId).gte("created_at", dateStr).lte("created_at", dateStr + "T23:59:59").limit(1);
      if (moodData && moodData.length > 0) {
        memories.push({ content: `You were feeling ${moodData[0].mood.toLowerCase()}`, yearsAgo, type: "mood", color: moodData[0].color });
      }

      const { data: dreamData } = await supabase.from("dreams").select("*").eq("user_id", userId).gte("created_at", dateStr).lte("created_at", dateStr + "T23:59:59").limit(1);
  ...
  const loadMonthlyMemories = async (userId: string) => {
    ...
    for (let i = 1; i <= monthsToLoad; i++) {
      ...
      const { data: msgs } = await supabase.from("messages").select("content").eq("user_id", userId).eq("role", "user").gte("created_at", startOfMonth).lte("created_at", endOfMonth).limit(1);
      const { data: moods } = await supabase.from("moods").select("mood, color").eq("user_id", userId).gte("created_at", startOfMonth).lte("created_at", endOfMonth);
      const { count } = await supabase.from("messages").select("*", { count: "exact", head: true }).eq("user_id", userId).eq("role", "user").gte("created_at", startOfMonth).lte("created_at", endOfMonth);
  ...
  const loadMoodJourney = async (userId: string) => {
  ...
  const loadDreams = async (userId: string) => {
    ...
    const { data } = await supabase.from("dreams").select("*").eq("user_id", userId).gte("created_at", startDate).order("created_at", { ascending: false }).limit(20);
    if (data) {
  ...
  const loadMonthlyWrappedData = async (userId: string) => {
    const now = new Date();
    const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1).toISOString();

    const { data: msgs, count: msgCount } = await supabase.from("messages").select("*", { count: "exact" }).eq("user_id", userId).eq("role", "user").gte("created_at", startOfMonth);
    const { data: moods, count: moodCount } = await supabase.from("moods").select("*", { count: "exact" }).eq("user_id", userId).gte("created_at", startOfMonth);
    const { count: dreamCount } = await supabase.from("dreams").select("*", { count: "exact", head: true }).eq("user_id", userId).gte("created_at", startOfMonth);
  ...
  const loadYearlyWrappedData = async (userId: string) => {
    const now = new Date();
    const startOfYear = new Date(now.getFullYear(), 0, 1).toISOString();

    const { data: msgs, count: msgCount } = await supabase.from("messages").select("*", { count: "exact" }).eq("user_id", userId).eq("role", "user").gte("created_at", startOfYear);
    const { data: moods, count: moodCount } = await supabase.from("moods").select("*", { count: "exact" }).eq("user_id", userId).gte("created_at", startOfYear);
    const { count: dreamCount } = await supabase.from("dreams").select("*", { count: "exact", head: true }).eq("user_id", userId).gte("created_at", startOfYear);
  ...
  const loadMiraRemembers = async (userId: string) => {
    const { data: msgs } = await supabase.from("messages").select("content, created_at").eq("user_id", userId).eq("role", "user").order("created_at", { ascending: false }).limit(50);
    ...
    <div key="y4" style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", textAlign: "center", padding: "40px" }}>
      ...
      {yearlyWrappedData.topPerson ? (
        <>
          ...
          <p style={{ color: "rgba(255,255,255,0.5)", fontSize: "16px", marginTop: "8px" }}>{yearlyWrappedData.topPersonCount} mentions</p>
        </>
      ) : <p style={{ fontSize: "20px", color: "rgba(255,255,255,0.5)" }}>Keep chatting!</p>}
    </div>,
      ...
      <main style={{ padding: "20px", maxWidth: "500px", margin: "0 auto", paddingBottom: "40px" }}>
        ...
        {isLoading ? (
          ...
          <>
              ...
              <section style={{ backgroundColor: "rgba(139,92,246,0.1)", borderRadius: "20px", padding: "20px", marginBottom: "20px", border: "1px solid rgba(139,92,246,0.2)" }}>
                ...
                {onThisDay.map((memory, i) => (
                  <div key={i} style={{ marginBottom: i < onThisDay.length - 1 ? "16px" : 0, paddingBottom: i < onThisDay.length - 1 ? "16px" : 0, borderBottom: i < onThisDay.length - 1 ? "1px solid rgba(255,255,255,0.05)" : "none" }}>
                    ...
                    <p style={{ color: memory.color || "rgba(255,255,255,0.85)", fontSize: "14px", lineHeight: 1.5 }}>{memory.content.slice(0, 120)}{memory.content.length > 120 ? "..." : ""}</p>
                  </div>
                ))}
              </section>
            )}
                    ...
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                      ...
                      <div style={{ flex: 1, display: "flex", height: "20px", borderRadius: "10px", overflow: "hidden" }}>
                        {month.moods.map((mood, j) => {
                          ...
                          return <div key={j} style={{ width: `${(mood.count / total) * 100}%`, backgroundColor: mood.color, minWidth: "4px" }} />;
                        })}
                      </div>
                    </div>
                  ))}
            ...
            {monthlyMemories.length === 0 && onThisDay.length === 0 && dreams.length === 0 && (
              <div style={{ textAlign: "center", padding: "40px 20px" }}>
                <p style={{ color: "rgba(255,255,255,0.5)", fontSize: "15px", marginBottom: "16px" }}>Keep using MIRA to build memories</p>
