...
export default function Reflect() {
  ...
  const loadRecentEntries = async () => {
    const { data: moodData } = await supabase.from("moods").select("*").order("created_at", { ascending: false }).limit(5);
    if (moodData) {
    ...
    const { data: msgData } = await supabase.from("messages").select("*").eq("role", "user").like("content", "[Moment]%").order("created_at", { ascending: false }).limit(3);
    if (msgData) {
    ...
    const { data: dreamData } = await supabase.from("dreams").select("*").order("created_at", { ascending: false }).limit(3);
    if (dreamData) {
  ...
  const saveMood = async (mood: string, color: string) => {
    ...
    setSaving(true);
    await supabase.from("moods").insert({ mood, color });
    setSaving(false);
  ...
  const saveMoment = async () => {
    ...
    setSaving(true);
    await supabase.from("messages").insert({ role: "user", content: `[Moment] ${moment}` });
    setMoment("");
    setSaving(false);
  ...
  const saveDream = async () => {
    ...
    setSaving(true);
    await supabase.from("dreams").insert({ dream_text: dream });
    setDream("");
    setSaving(false);
